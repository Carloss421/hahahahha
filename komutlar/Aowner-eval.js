...
//var code2 = args.slice(1).join(' ') ||  args.join(' ');
 ...
 if(code.match(/(client.token)/g)) {
  ...
  .setColor('RANDOM')
    .setDescription(`${lang.evalCommand.AownerEvalERROR}\n\`\`\`xl\n${tokenuyari}\`\`\``)
    message.channel.send(token_uyari);
     return;
 };
if (args[0] === "ayarlar.token") code=lang.evalCommand.AownerEvalTOKENERRORED
if (args[0] === "config.token") code=lang.evalCommand.AownerEvalTOKENERRORED
 if(code.match(/(client["token"])/g)) {
 ...
 function clean(text) {
    if (typeof text !== 'string')
     ...
     .setColor('RANDOM')
         .setDescription(`${lang.evalCommand.AownerEvalCODE}\n\`\`\`javascript\n${code}\n\`\`\``)
         .setDescription(`${lang.evalCommand.AownerEvalCONCLUSION}\n\`\`\`xl\n${evaled}\`\`\``)
         message.channel.send(giren_cıkan);
    } catch (err) {
    embed.setColor('RANDOM')
        embed.setDescription(`${lang.evalCommand.AownerEvalERROReD}\n\`\`\`xl\n${err}\n\`\`\``)
