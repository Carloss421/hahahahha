...
//let kayıtye = db.fetch(`kayıty_${message.guild.id}`)
let kayıtlg = db.fetch(`logkayıt_${message.guild.id}`)
let normalr = db.fetch(`norml_${message.guild.id}`)
let alnck = db.fetch(`kayıtalınacakrol_${message.guild.id}`)
let yetkili = db.fetch(`kayıt_.${message.guild.id}`)
...
.addField(`**Kayıt Yetkili Rolü**`, `${db.has(`kayıty_${message.guild.id}`) ? `<@&${yetkili}>` : `\`Ayarlanmamış!\``}`,true)
