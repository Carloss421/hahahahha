...
animEmotes = animEmotes.length !== 0 ? `\n\n__**[${animEmotes.length}] Hareketli Emoji**__\n${animEmotes.join('')}` : '\n**Hareketli Emoji Bulunmuyor**';
        try {     
let botembed = new Discord.MessageEmbed().setColor(`RANDOM`)
.setDescription(`${staticEmotes}${animEmotes}`).setAuthor(`${msg.guild.name} Sunucusu Emojileri`, msg.guild.iconURL())
                           .setFooter('Bu komutu kullanan kullanıcı ' + msg.author.tag, msg.author.avatarURL()).setTimestamp()
        return msg.channel.send(botembed)
      } catch (err) {
        const embed = new Discord.MessageEmbed()
            .addField(`${err}`/*`Sunucuda Bulunmayan Emojiler`, 'Üzgünüm ama sunucunuzda ya çok fazla emoji bulunuyor ya da hiç emoji bulunmuyor. Bunları gösteremiyorum. Discord buna izin vermiyor.'*/)
            .setColor('RED')
