...
.setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
.setTitle("BAŞARILI!")
.setDescription(`<:ekle0:838855131495268414> ${etiket} **${para}TL** eklendi!\nEkleyen: ${message.author}`))
...
.setDescription(`🤔 Girdiğin miktar geçerli bir sayı değil !?`))
  ...
  if(miktar > para) return message.channel.send(new Discord.MessageEmbed()
                      .setColor("RED")
                      .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))                 
                      .setDescription(`⛔ Şuan elinde ${para ? "sadece **" + para + "TL** var!": "hiç para yok!"} `))
if(miktar === 'all' || miktar === 'hepsi') {
  return;
}  else { 
  message.channel.send(new Discord.MessageEmbed()
                ...
                .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))   
                .setTitle("BAŞARILI!")
                .setDescription(`<:ekle0:838855131495268414> ${etiket} hesabına **${miktar}TL** eklendi!\nEkleyen: <@${message.author.id}>`))
