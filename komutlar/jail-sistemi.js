...
\`${client.ayarlar.prefix}jail-log-kapat\`
\`${prefix}jail-yetkili-role [@rolEtiket]\`
Jaili **kullanacak yetkili** rolünü etiketle
\`${prefix}jail-karantina-role [@rolEtiket]\`
Jail **karantina rolünü** etiketle`)
.addField('ᅠ', 'ᅠ')
.addField('Jail Komutu', `${prefix}cezalı, ${prefix}Cezalı, ${client.ayarlar.prefix}jail`)
.addField('Örnek Karantina komut', `\`\`\`${prefix}jail @etiket\`\`\``)
.addField('ᅠ', 'ᅠ')// xd
.addField('Jail den çıkarmak isterseniz', `\`${client.ayarlar.prefix}k-erkek\`, \`${client.ayarlar.prefix}k-kadın\`\n**Komutları aktif etmek için:** \`${prefix}erkek-role [@roleEtiket]\` **-** \`${client.ayarlar.prefix}kadın-role [@roleEtiket]\``)
