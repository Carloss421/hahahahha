...
const data = await fetch.get(`http://www.googleapis.com/youtube/v3/channels?part=snippet,contentDetails,statistics,brandingSettings&id=${id}&key=${id}`)
...
.catch(() => console.log("!!! Kanal bulunamadı, yeniden dene. !!!"));
      ...
      message.channel.send(name + "YouTube`" + parseInt(data.body.items[0].statistics.subscriberCount).toLocaleString() + "` Aboneye ulaştı")
  */
